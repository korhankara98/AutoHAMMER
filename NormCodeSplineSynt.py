...
fits_file = '/Users/korhankara/Desktop/MScStellae/delOri/3800_7800_feros/delOri_4200-4500_feros.fits'
...
def normalize_spectrum(wavelength, flux, selected_wavelengths, selected_fluxes, s_factor=1e-6):
    """
    Tayfı normalize etmek için seçilen noktalara spline fit uygular ve akıyı bu eğriye böler.
    """
    ...
    # Spline derecesi
    k = min(2, len(sorted_wavelengths)-1)
    if k < 1:
        print("Spline fit için yeterli nokta yok.")
        return flux, np.ones_like(flux)

    spline = UnivariateSpline(sorted_wavelengths, sorted_fluxes, k=k, s=s_factor * len(sorted_wavelengths))
    continuum_fit = spline(wavelength)
...
if wavelength is None:
    original_fits_file = '/Users/korhankara/Desktop/MScStellae/delOri/3800_7800_feros/delOri_4200-4500_feros.fits'
...
line1, = ax1.plot(wavelength, flux, label='Akı vs Dalga Boyu')
normalization_line, = ax1.plot([], [], 'r--', label='Normalizasyon Eğrisi')
normalized_line, = ax2.plot([], [], 'g-', label='Normalize Edilmiş Akı', linewidth=2)

data_file = '/Users/korhankara/Desktop/delOri_4200-4500.10'
...
try:
    ...
    ax2.plot(wavelength_file, flux_file, 'b-', label='Synthetic Data')
...
def remove_closest_point(x, y):
    if selected_wavelengths and selected_fluxes:
        ...
        if distances[closest_idx] < 0.1:  # Yakınlık eşiği
...
def update_normalization_line():
    if len(selected_wavelengths) >= 2:
        ...
        # Spline
        spline = UnivariateSpline(sorted_wavelengths, sorted_fluxes, k=k, s=1e-6 * len(sorted_wavelengths))
        continuum_fit = spline(wavelength)
...
def on_key(event):
    ...
    if event.key == 'x':  # Nokta seçimi yapar
        ...
        if len(selected_wavelengths) >= 2:
            ...
            new_fits_file = '/Users/korhankara/Desktop/MScStellae/delOri/3800_7800_feros/delOri_4200-4500_feros_Norm.fits'
            ...
            txt_file = '/Users/korhankara/Desktop/MScStellae/delOri/3800_7800_feros/delOri_4200-4500_feros_Norm.txt'
...
plt.plot(wavelength_norm, normalized_flux_data, 'g-', label='Normalize Akı (IRAF uyumlu)')
plt.xlabel('Dalga Boyu')
plt.ylabel('Normalize Edilmiş Akı')
plt.title('Dalga Boyu - Normalize Akı')
plt.legend()
plt.grid(True)
plt.show()
