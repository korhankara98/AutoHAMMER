...
def normalize_spectrum(wavelength, flux, selected_wavelengths, selected_fluxes):
    """
    Tayfı normalize etmek için seçilen noktalara Akima interpolasyonu uygular ve akıyı bu eğriye böler.
    """
    ...
    if len(sorted_wavelengths) < 5:
        print("Akima interpolasyonu için yeterli nokta yok.")
        return flux, np.ones_like(flux)

    akima_interp = Akima1DInterpolator(sorted_wavelengths, sorted_fluxes)
    continuum_fit = akima_interp(wavelength)
...
def on_key(event):
    ...
    if event.key == 'x':  # Nokta seçimi yapar
        ...
        if len(selected_wavelengths) >= 5:
            ...
            np.savetxt(txt_file, np.column_stack((wavelength, normalized_flux)), header='Wavelength Normalized_Flux', fmt='%.6f')
...
ax1.set_ylabel('Akı')
ax1.set_title('Dalga Boyu vs Akı ve Normalizasyon Eğrisi Akima')
ax1.legend()
ax1.grid(True)
...
plt.title('Dalga Boyu - Normalize Akı Akima')
plt.legend()
plt.grid(True)
plt.show()